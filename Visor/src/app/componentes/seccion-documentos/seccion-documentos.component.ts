...
export class SeccionDocumentosComponent implements OnInit {
  ...
  buscarFiltro() {
    this._filtroService
      ...
      .subscribe((response: any) => {
        ...
        this._filtroService
          .selectFiltro(response.dataset[0].id, 'documentos')
          .subscribe((resp: any) => {
          ...
          .subscribe((result) => {
            ...
            for (let i = 0; i < result.dataset.length; i++) {
              let document: Documento = {
                ...
                active_date:
                  result.dataset[i].active_date.slice(8, 10) +
                  '-' +
                  result.dataset[i].active_date.slice(5, 7) +
                  '-' +
                  result.dataset[i].active_date.slice(0, 4),
  ...
  openFilter() {
    ...
    const dialogRef = this.dialog.open(ModalFiltroDocumentosComponent, {
      ...
      data: {
        ...
        estado: this.estado,
        publicacionDesde: this.fechaPublicacionDesde,
        publicacionHasta: this.fechaPublicacionHasta,
        caducidadDesde: this.fechaCaducidadDesde,
        caducidadHasta: this.fechaCaducidadHasta,
  ...
  buscarDocumentos(event: Event) {
    ...
    this.buscarFiltro();
    this.documentService
      .getDocumento(this.valorInputDocumento)
      .subscribe((result) => {
    ...
    if (this.orden_saved_search_id == '') {
      this._filtroService
        .insertFiltro(
          ...
          'filtro_orden',
          encodedData,
          'Filtra los documentos por orden alfabetico o fecha'
        )
        .subscribe((rsp: any) => {
      ...
      if (this.assigned_saved_search_id == '') {
        this._filtroService
          .insertFiltro(
            ...
            encodedData,
            'Filtra los documentos asignados al usuario'
          )
          .subscribe((rsp: any) => {
            console.log('Filtro guardado: ', rsp);
          });
        ...
        this._filtroService
          .updateFiltro(this.assigned_saved_search_id, encodedData)
          .subscribe((rsp: any) => {
            console.log('Filtro actualizado: ', rsp);
          });
      }
  ...
  abrirEditarDocumento(documento: Documento) {
    ...
    this.dialog.open(ModalDocumentosComponent, {
      ...
      data: {
        ...
        proyectoAsociado: documento.name,
        tipo: documento.type,
        estado: documento.status,
        fechaPublicacion: documento.active_date,
        fechaCaducidad: documento.finishedDate,
        asignadoA: documento.user_name,
      },
    });
  }
}
